    ...
    <li key={index} className="mb-4">
                                ...
                                <div className="flex flex-row flex-wrap items-center justify-center  gap-4 mt-5 w-[60%] mx-auto">
                                    {question.options.map((option, optionIndex) => (
                                        <label key={optionIndex} className={`mb-2 p-3 border-2 w-[40%] cursor-pointer rounded-lg ${selectedOptions.find((q) => (q.selectedOption === option && q.questionId ==question._id)) ? 'bg-green-600' : ''} hover:bg-green-600`}>
                                            <input
                                                type="radio"
